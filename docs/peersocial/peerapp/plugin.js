                ...
                init: function() {
                    ...
                    imports.state.$hash.on("/peerapp-view", async(query) => {
                        ...
                        imports.user.getUser(query[1], query[0], async function(err, $user, user) {
                            ...
                            var layout = $(await imports.app.layout.ejs.render(require("./appView.html"), { appSource: await user.get('profile').get("peerappsDev").get(id).get("appSource") }, { async: true }))
                    ...
                    imports.state.$hash.on("/peerapp-edit", async(query) => {
                        ...
                        imports.user.getUser(query[1], query[0], async function(err, $user, user) {
                            ...
                            var layout = $(await imports.app.layout.ejs.render(require("./appEdit.html"), { appSource: await user.get('profile').get("peerappsDev").get(id).get("appSource") }, { async: true }))
                    ...
                    imports.state.$hash.on("/peerapp", function() {
                        if (imports.gun.user().is) {
                            imports.user.me(async function(err, me, user) {
                                ...
                                var layout = $(await imports.app.layout.ejs.render(require("./appList.html"), { me: me, user: user, peerapps: peerapps, peerappsDev: peerappsDev }, { async: true }))
                                ...
                                layout.find("#createApp").click(() => {
                                    ...
                                    var model = $(require("./createApp.html"));
