define(function(require, exports, module) {
    ...
    function appPlugin(options, imports, register) {
        ...
        function peerConfirm(text,done){
            ...
            var loginModel = $(imports.app.layout.ejs.render(require("text!./peer-confirm.html"),{
                body:text
            }));
                ...
                if (!err) {
                    ...
                    _self.listPeers(function(pList){
                        for (var i in pList) {
                            ...
                            var peerCard = $(imports.app.layout.ejs.render(require("text!./peer-card.html"), { profile:pList[i].profile, bio: pList[i].bio, profileImage:pList[i].profileImage, alias: pList[i].alias, pub: pList[i].pub, uid32: pList[i].uid32, add: false }));
                        ...
                        for (var i in peersL) {
                            if (i != "_" && i != "#")
                                getPeerData(peersL[i],function(data,profile) {
                                    ...
                                    var peerCard = $(imports.app.layout.ejs.render(require("text!./peer-card.html"), { profile:profile, bio: data.bio, profileImage:data.profileImage, alias: data.alias, pub: data.pub, uid32: data.uid32, add: false }));
                                    peerListLayout.find(".peer-cards").append(peerCard);
                                });
                        }
                    });*/
                    ...
                    peerListLayout.find("#loopupPeer").click(() => {
                        var addedPeer = false;
                        var loginModel = require("text!./loopupPeer.html");
                        var model = $(loginModel);
                        model.modal({
                            show: true
                        });
                            ...
                            if(username && userid32){
                                gun.user("@" + username).once(async(data) => {
                                    ...
                                    if ($$peer) {
                                        ...
                                        var peerCard = $(await imports.app.layout.ejs.render(require("text!./peer-card.html"), { bio: "", profileImage:$$peer.profileImage, alias: $$peer.alias, pub: $$peer.pub, uid32: $$peer.uid32, add: true }, { async: true }));
