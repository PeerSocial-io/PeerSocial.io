;(function(sr) {
  ...
  function the() { // THIS CODE RUNS INSIDE THE WEBWORKER!
    ...
    var up = postMessage,
      ...
      perf, u;
    up.s = [];
    perf = this.performance || { now: function() { return +new Date } };
    ...
    sr.how.view = function(list) {
      ...
      function each(change, name, what, has, put, text, tmp) {
        ...
        if (u !== (put = change.grab)) {
          if (u !== put[2]) { change.zoom = [put[2], put[2], what.zoom[2]];
            put[2] = 0; } // simulate 3D by converting Z to zoom.
          tmp = what.grab;
          var j = -1,
            l = put.length;
          while (++j < l) {
